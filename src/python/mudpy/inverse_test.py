...
def getLs(home,project_name,fault_name,nfaults,num_windows,bounds,segs):
    '''
    ...
    for i in range(len(nstrike)):
        print 'Working on segment of length' + nstrike[i].astype(str)
        for kfault in range(N):#Loop over faults and fill regularization matrix
            stencil,values=laplace_stencil(kfault,nstrike[i],ndip,bounds,segs)
...
def laplace_stencil(ifault,nstrike,ndip,bounds,segs):
    '''
...
def model_covariance(home,project_name,run_name,run_number,fault_name,G_name,nfaults,
                        num_windows,bounds,GF_list,decimate,lowpass,beta,segs):
    '''
    ...
    Ls=getLs(home,project_name,fault_name,nfaults,num_windows,bounds,segs)
